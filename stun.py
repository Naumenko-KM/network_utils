            ...
            if not ret['Resp']:
                ...
                if exIP == ret['ExternalIP'] and exPort == ret['ExternalPort']:
                    changePortRequest = ''.join([ChangeRequest, '0004',
                                                 "00000002"])
                    log.debug("Do Test3, probe2")
                    ret = stun_test(s, changedIP, changedPort, source_ip, source_port,
                                    changePortRequest)
                    log.debug("Result: %s", ret)

                    log.debug("Do Test3, probe2")
                    ret2 = stun_test(s, changedIP, changedPort, source_ip, source_port,
                                    changePortRequest)
                    log.debug("Result: %s", ret)

                    log.debug("Do Test3, probe3")
                    ret = stun_test(s, changedIP, port, source_ip, source_port)
                    log.debug("Result: %s", ret)

                    log.debug("Do Test3, probe4")
                    ret = stun_test(s, changedIP, changedPort, source_ip, source_port,
                                    changePortRequest)
                    log.debug("Result: %s", ret)
                    if ret2['Resp']:
